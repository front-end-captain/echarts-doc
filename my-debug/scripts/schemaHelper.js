...
function convertToTree(rootSchema, rootNode) {
  ...
  function createArrayItemNode(schema, parentNode) {
    ...
    if (
      ...
      schema.properties.type.default
    ) {
      childNode.arrayItemType = schema.properties.type.default.replace(
        /'/g,
        ""
      );
    } else {
...
module.exports.extractDesc = function(schema, docName) {
  ...
  console.log(
    `Options with UIControl ${propWithUIControlCount} / ${propTotalCount} (${propWithUIControlCount /
      propTotalCount})`
  );
